  ...
  <div class="slider-div">
    <div class="setting-head">
      ...
      <template v-for="(item, index) in showTags" :key="index">
        <div
          :class="activeTag === item ? 'title-div-active' : 'title-div'"
...
const addShowTag = () => {
  showIndex.value =
    showIndex.value + 4 < titleList.value.length
      ? showIndex.value + 4
      : Math.floor((titleList.value.length - 1) / 4) * 4;
};
...
onMounted(() => {
  showTags.value = titleList.value
    .slice(showIndex.value, showIndex.value + 4)
    .map(item => {
      return item.name;
    });
});
watch(showIndex, () => {
  showTags.value =
    showIndex.value + 4 < titleList.value.length
      ...
      : titleList.value
          .slice(showIndex.value, titleList.value.length)
          .map(item => {
            return item.name;
          });
});
